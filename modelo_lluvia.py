...
def obtener_datos_horarios(lat, lon, api_key):
    if not api_key or not str(api_key).strip():
        st.error("Falta la clave de API: define `api` en el archivo `.env` junto a esta app.")
        return None
    url = f"https://api.openweathermap.org/data/2.5/forecast?lat={lat}&lon={lon}&appid={api_key}&units=metric"
    try:
        response = requests.get(url, timeout=20)
    except requests.RequestException as e:
        st.error(f"No se pudo conectar con OpenWeather: {e}")
        return None
...
if clima_df is not None:
    ...
    if not clima_df.empty:
        ...
        st.image(imagen_clima, caption=texto_clima, width="stretch")
    else:
